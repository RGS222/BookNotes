...
export async function getUserIdPassword(db, name) {
    // Pass NULL as a placeholder for the OUT parameter
    const query = "CALL getUserIdPassword($1, NULL)";
    const values = [name];
    ...
    try {
        ...
        // The OUT parameter values are returned in the first row
        if (res.rows[0]) {
            return [res.rows[0]._id, res.rows[0]._password];
        } else {
            return [];
        }
    } catch (err) {
        console.error("Error executing procedure getUser:", err);
        return null;
    }
}
